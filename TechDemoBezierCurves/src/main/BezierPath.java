...
//				if(command.contains(rayVector))	//Check first if directionVector intersects current bounds2D of the current command
//				{
					ArrayList<double[]> currentIntersectionPoints = command.calculatePossibleIntersectionPoints(rayVector);
					intersectionPoints.addAll(currentIntersectionPoints);
//				}
			}
		}
	...
	private abstract class BezierCommand implements BezierIPathCommand
	{
		...
		@Override
		public final double[] calculateIntersectionPoint(double[][] rayVector) 
		{
			ArrayList<double[]> possibleIntersectionPoints = this.calculatePossibleIntersectionPoints(rayVector);
			double[] intersectionPoint = getClosestIntersectionPoint(possibleIntersectionPoints,rayVector);
			return intersectionPoint;
		}
		
		@Override
		public abstract ArrayList<double[]> calculatePossibleIntersectionPoints(double[][] rayVector);
	...
	private class LineToCommand  extends BezierCommand implements BezierIPathCommand
	{
		...
		@Override
		public ArrayList<double[]> calculatePossibleIntersectionPoints(double[][] rayVector)   //TODO Test this!
		{
			/*
			 * RayVector:
